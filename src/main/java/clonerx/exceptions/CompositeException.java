 ...
 * 代表由一个或者多个异常组合的异常，该异常并不修改它所包含的异常结构，不过在打印的时候会通过迭代的方法，打印所有的异常
 *
 * 它的确定性是通过包含一个不可变的、有序的（通过插入排序）、唯一的非组合异常实现的，你可以通过{@link #getExceptions()}获取该列表
 * 为了防止循环引用，{@link #printStackTrace()}使用自定义的堆栈跟踪方法去代替{@code getCause()}.
 *
 * 如果调用{@code getCause()}, 它会延迟创建调用链，如果发现已经在此调用链中，则会停止创建
 */
